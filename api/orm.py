...
from api.utils import gen_oid, gen_now
...
class FaunaModel(BaseModel):
    ...
    def find(cls, oid: str) -> Optional[Dict[str, Any]]:
        ...
        try:
            setref = cls.q()(
                q.match(q.index(f"{cls.__name__.lower()}_oid_unique"), oid)
            )
    ...
    def delete(cls, oid: str) -> bool:
        ...
        try:
            ref = cls.q()(
                q.get(q.match(q.index(f"{cls.__name__.lower()}_oid_unique"), oid))
            )
            cls.q()(q.delete(ref["ref"]))
            return True
        except NotFound:
    ...
    def update(self) -> Dict[str, Any]:
        ...
        try:
            ref = _q(
                q.get(
                    q.match(q.index(f"{self.__class__.__name__.lower()}_id"), self.oid)
                )
            )
